import { useEffect, useRef } from "react";

const M4Mask = () => {
    const containerRef = useRef<HTMLDivElement>(null);
    const groupRef = useRef<SVGGElement>(null);

    useEffect(() => {
        const handleScroll = () => {
            if (!containerRef.current || !groupRef.current) return;
            const rect = containerRef.current.getBoundingClientRect();
            const windowH = window.innerHeight;

            // Start zoom earlier (when the video container is 300px below the top of the screen)
            const startThreshold = 2000;
            
            if (rect.top > startThreshold) {
                groupRef.current.style.transform = "scale(60)";
                return;
            }

            // Calculate progress through the scroll track, starting from startThreshold
            const totalScrollable = rect.height - windowH;
            const currentScroll = startThreshold - rect.top;
            const totalScroll = totalScrollable + startThreshold;
            const progress = Math.max(0, Math.min(1, currentScroll / (totalScroll || 1)));

            // Map progress to animation states:
            // - 0.00 -> 0.02: video remains fully full-screen (scale = 60)
            // - 0.02 -> 0.85: mask zooms in slowly and smoothly from 60 to 1.0 based on scroll travel
            // - 0.85 -> 1.00: mask stays locked at 1.0
            let animProgress = 0;
            if (progress < 0.02) {
                animProgress = 0;
            } else if (progress > 0.85) {
                animProgress = 1;
            } else {
                animProgress = (progress - 0.02) / 0.83;
            }

            // Custom cubic ease curve to keep zoom fluid and natural
            const easedProgress = Math.pow(animProgress, 3);
            const scale = 60.0 - (60.0 - 1.0) * easedProgress;

            // Update scale transform directly for 60fps performance
            groupRef.current.style.transform = `scale(${scale})`;
        };

        window.addEventListener("scroll", handleScroll, { passive: true });
        handleScroll(); // Initial call

        return () => window.removeEventListener("scroll", handleScroll);
    }, []);

    return (
        <div ref={containerRef} className="relative w-screen h-[480vh] bg-black">
            {/* Sticky screen-height container */}
            <div className="mt-20 sticky top-0 left-0 w-screen h-screen overflow-hidden flex items-center justify-center bg-black">
                {/* Background Fullscreen Video */}
                <video
                    src="/videos/game.mp4"
                    autoPlay
                    loop
                    muted
                    playsInline
                    className="absolute top-0 left-0 w-screen h-screen object-cover z-0"
                />
                
                {/* SVG Reverse Mask Overlay */}
                <svg
                    viewBox="0 0 1920 1080"
                    preserveAspectRatio="xMidYMid slice"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                    className="absolute top-0 left-0 w-screen h-screen z-10 pointer-events-none"
                >
                    <g 
                        ref={groupRef}
                        style={{
                            transform: "scale(60)",
                            transformOrigin: "960px 540px",
                            transformBox: "view-box",
                            transition: "transform 0.15s cubic-bezier(0.1, 0.8, 0.2, 1.0)",
                            willChange: "transform"
                        }}
                    >
                        {/* Apple M4 logo cutout path */}
                        <path 
                            fillRule="evenodd"
                            d="M1364.68 484.97C1358.2 495.27 1351.68 505.61 1345.12 515.98C1338.56 526.35 1332.01 536.72 1325.46 547.09C1318.9 557.46 1312.35 567.8 1305.8 578.1C1299.24 588.4 1292.73 598.74 1286.24 609.11V610.84H1385.64V453.96H1384.13C1377.65 464.33 1371.16 474.67 1364.68 484.97Z" 
                            fill="black"
                        />
                        <path 
                            fillRule="evenodd"
                            d="M0 0V1080H1920V0H0ZM614.8 351.95C619.66 341.4 625.17 332.66 631.33 325.75C639.18 316.41 649.08 308.66 661.03 302.49C672.98 296.32 684.38 293.06 695.22 292.68C695.78 295.11 696.06 298 696.06 301.37C696.06 312.58 693.91 323.46 689.62 334.02C685.32 344.58 679.81 353.96 673.09 362.18C665.8 370.96 656.65 378.3 645.63 384.18C634.61 390.06 624.15 393.01 614.24 393.01C612.18 393.01 610.22 392.83 608.36 392.45C608.17 391.89 607.98 390.86 607.8 389.37C607.61 387.88 607.52 386.29 607.52 384.6C607.51 373.39 609.94 362.51 614.8 351.95ZM778.59 631.48C774.01 644 766.49 657.92 756.03 673.24C746.87 686.51 737.02 698.74 726.46 709.95C715.9 721.16 703.25 726.76 688.49 726.76C678.59 726.76 670.46 725.31 664.11 722.42C657.76 719.52 651.22 716.63 644.49 713.73C637.76 710.84 628.7 709.39 617.31 709.39C606.28 709.39 597.08 710.89 589.71 713.87C582.33 716.86 575.32 719.85 568.69 722.84C562.06 725.83 554.26 727.32 545.29 727.32C531.65 727.32 519.69 721.9 509.42 711.07C499.14 700.24 488.59 687.25 477.75 672.12C465.23 654.18 454.53 632.28 445.66 606.4C436.79 580.53 432.35 554.42 432.35 528.08C432.35 499.87 437.67 476.19 448.32 457.04C458.97 437.89 472.65 423.41 489.37 413.61C506.09 403.8 523.42 398.9 541.35 398.9C550.88 398.9 559.84 400.44 568.25 403.52C576.66 406.6 584.55 409.73 591.93 412.91C599.31 416.09 605.99 417.68 611.97 417.68C617.76 417.68 624.48 416 632.15 412.63C639.81 409.27 648.4 405.95 657.93 402.68C667.46 399.41 677.64 397.77 688.48 397.77C692.59 397.77 699.59 398.43 709.5 399.73C719.4 401.04 730.28 404.73 742.15 410.8C754.01 416.87 764.8 426.91 774.52 440.92C773.96 441.48 771.25 443.49 766.39 446.95C761.53 450.41 756.11 455.45 750.14 462.08C744.16 468.71 738.93 477.12 734.45 487.3C729.97 497.48 727.72 509.58 727.72 523.59C727.72 539.66 730.57 553.29 736.27 564.5C741.97 575.71 748.6 584.72 756.17 591.54C763.74 598.36 770.46 603.36 776.35 606.53C782.23 609.71 785.36 611.39 785.74 611.57C785.55 612.33 783.17 618.97 778.59 631.48ZM1182.41 718.44H1124.07V507.77H1122.38L1038.29 718.45H997.23L913.14 507.77H911.23V718.45H853.1V406.65H928.51L1017.1 630.72H1018.4L1106.78 406.65H1182.41V718.44ZM1487.63 661.4H1446.58V718.45H1384.35V661.4H1229.42V606.73C1237.63 593.48 1245.91 580.15 1254.27 566.76C1262.62 553.36 1270.98 540 1279.33 526.68C1287.68 513.36 1296.04 500.03 1304.39 486.71C1312.74 473.39 1321.1 460.03 1329.45 446.63C1337.8 433.23 1346.09 419.91 1354.3 406.66H1446.56V609.13H1487.61V661.4H1487.63Z" 
                            fill="black"
                        />
                    </g>
                </svg>
            </div>
        </div>
    );
};

export default M4Mask;
